...
- If a memory is directly contradicted by new information, critically evaluate both pieces of information:
    - If the new memory provides a more recent or accurate update, replace the old memory with new one (UPDATE).
    - If the new memory seems inaccurate or less detailed, retain the old memory and discard the new one (DO NOT UPDATE).
